(ns rockwell_factorytalk.main-test
  (:require [clojure.test :refer [deftest is testing run-tests]]
            [clojure.string :as str]
            [rockwell_factorytalk.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "rockwell_factorytalk-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))
