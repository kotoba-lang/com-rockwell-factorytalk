(ns rockwell_factorytalk.main
  "Kotodama WASM entrypoint for the Rockwell_factorytalk clean-room actor (L4) — Clojure port.
  ...
  facts (`rockwell_factorytalk.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rockwell_factorytalk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "rockwell_tag"
    :fields [:address :name :value :quality :unit]  :required [:address :name]
    :coerce {:value :float}              :refs {}}
   {:entity "Device"     :plural "devices"     :id-prefix "rockwell_dev"
    :fields [:name :protocol :ipAddress :online]    :required [:name :protocol]
    :coerce {:online :bool}              :refs {}}
   {:entity "Alarm"      :plural "alarms"      :id-prefix "rockwell_ala"
    :fields [:tagId :priority :state :raisedAt]     :required [:priority :state]
    :coerce {}                           :refs {:tagId "Tag"}}
   {:entity "Trend"      :plural "trends"      :id-prefix "rockwell_tre"
    :fields [:tagId :sampleRateMs :retentionDays]   :required [:sampleRateMs :retentionDays]
    :coerce {:sampleRateMs :int :retentionDays :int} :refs {:tagId "Tag"}}
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "rockwell_con"
    :fields [:name :setpoint :processValue :mode]   :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float} :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "rockwell_bat"
    :fields [:recipe :status :startedAt]            :required [:recipe :status]
    :coerce {}                           :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"rockwell_factorytalk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rockwell_factorytalk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rockwell_factorytalk-compat/ready)
